...
public:
    ...
    uint32_t getDescriptor(uint8_t* buffer, uint32_t bufferSize, uint32_t interface_id) const {
    	*reinterpret_cast<Eni::USB::InterfaceDescriptor*>(buffer) = getInterfaceDescriptor();
    	buffer += sizeof(Eni::USB::InterfaceDescriptor);
        ...
        for(uint8_t i = 0;; ++i){
            ...
            buffer += sizeof(Eni::USB::EndpointDescriptor);
        }
        ...
        return sizeof(Eni::USB::InterfaceDescriptor) + getInterfaceDescriptor().bNumEndpoints * sizeof(Eni::USB::EndpointDescriptor);
    }
    ...
    static void makeEndpointDescriptor(uint8_t* buffer, const UsbEndpoint* ep){
    	*reinterpret_cast<Eni::USB::EndpointDescriptor*>(buffer) = ep->getDescriptor();
    }
